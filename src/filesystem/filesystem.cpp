...
namespace {
    ...
    perms make_permissions(const path& p, DWORD attr)
    {
        ...
        if ((attr & FILE_ATTRIBUTE_READONLY) == 0)
            prms = (perms)((int)prms | (int)perms::owner_write | (int)perms::group_write | (int)perms::others_write);
        if (_stricmp(p.extension().string().c_str(), ".exe") == 0 || _stricmp(p.extension().string().c_str(), ".com") == 0 || _stricmp(p.extension().string().c_str(), ".bat") == 0 || _stricmp(p.extension().string().c_str(), ".cmd") == 0)
            prms = (perms)((int)prms | (int)perms::owner_exec | (int)perms::group_exec | (int)perms::others_exec);
        return prms;
    }
...
#define IO_REPARSE_TAG_SYMLINK (0xA000000CL)
    ...
    bool is_reparse_point_a_symlink(const path& p)
    {
        ...
        return reinterpret_cast<const REPARSE_DATA_BUFFER*>(buf.get())->ReparseTag == IO_REPARSE_TAG_SYMLINK
            ...
            || reinterpret_cast<const REPARSE_DATA_BUFFER*>(buf.get())->ReparseTag == IO_REPARSE_TAG_MOUNT_POINT; // aka "directory junction" or "junction"
    }
#else
    bool not_found_error(int errval)
    {
...
bool equivalent(const path& p1, const path& p2)
{
    ...
    return info1.dwVolumeSerialNumber == info2.dwVolumeSerialNumber && info1.nFileIndexHigh == info2.nFileIndexHigh && info1.nFileIndexLow == info2.nFileIndexLow && info1.nFileSizeHigh == info2.nFileSizeHigh && info1.nFileSizeLow == info2.nFileSizeLow && info1.ftLastWriteTime.dwLowDateTime == info2.ftLastWriteTime.dwLowDateTime && info1.ftLastWriteTime.dwHighDateTime == info2.ftLastWriteTime.dwHighDateTime;
