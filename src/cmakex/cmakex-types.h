...
struct pkg_desc_t {
    ...
    pkg_clone_pars_t c;
    pkg_build_pars_t b;
    std::map<string, string> deps_shas; // sha's of dependencies at the time of the build
};

struct installed_config_desc_t {
    static installed_config_desc_t uninitialized_installed_config_desc() { return installed_config_desc_t(); }
    ...
    std::map<string, string> deps_shas; // sha's of dependencies at the time of the build
private:
    installed_config_desc_t()
        : config(config_name_t::uninitialized_config_name())
    {
    }
};
...
struct pkg_request_details_against_installed_config_t {
    pkg_request_details_against_installed_config_t(const installed_config_desc_t& installed_config_desc)
...
//key is the config
using pkg_request_details_against_installed_t = std::map<config_name_t, pkg_request_details_against_installed_config_t>;
}
