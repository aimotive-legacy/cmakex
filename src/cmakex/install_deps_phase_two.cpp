...
void install_deps_phase_two(string_par binary_dir, deps_recursion_wsp_t& wsp)
{
    ...
    for (auto& kv : wsp.pkg_map)
        kv.second.request_eval = installdb.evaluate_pkg_request(kv.second.planned_desc);
    for (auto& p : wsp.build_order) {
        // explain and build if it's already been decided
        string build_reason;
        auto& wp = wsp.pkg_map[p];
        auto& re = wp.request_eval;
        switch (re.status) {
            case pkg_request_satisfied:
                break;
            case pkg_request_missing_configs:
                build_reason =
                    stringf("missing configurations (%s)", join(re.missing_configs, ", ").c_str());
                break;
            ...
            case pkg_request_not_compatible:
                build_reason =
                    stringf("build options changed (%s)", re.incompatible_cmake_args.c_str());
                break;
            default:
                CHECK(false);
        }
        if (build_reason.empty()) {
            ...
            if (clone_helper.cloned) {
                ...
                } else if (clone_helper.cloned_sha != re.pkg_desc.c.git_tag) {
            ...
            if (build_reason.empty()) {
                ...
                for (auto& d : wp.planned_desc.depends) {
                    ...
                    auto dep_orig_hash = re.pkg_desc.dep_shas.at(d);
